...
function render(vnode, container, praentComponent) {
...
function patch(vnode, container, praentComponent) {
    ...
    else if (1 /* ShapeFlags.ELEMENT */ & shapeFlags) {
        // dom
...
function processElement(vnode, container, praentComponent) {
    // 这里分为初始化（mount）和更新（update）
    mountElement(vnode, container, praentComponent);
}
// 处理element类型
function mountElement(vnode, container, praentComponent) {
    // vonde => element => div
...
// 处理frament slot
function processFragment(vnode, container, praentComponent) {
